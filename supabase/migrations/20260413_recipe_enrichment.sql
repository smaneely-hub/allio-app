...
SET yield_text = COALESCE(yield_text, CASE WHEN servings IS NOT NULL THEN servings::text || ' servings' ELSE NULL END),
    ...
    ingredient_groups_json = CASE
      WHEN ingredient_groups_json IS NULL OR ingredient_groups_json = '[]'::jsonb THEN jsonb_build_array(jsonb_build_object('ingredients', COALESCE(ingredients_json, '[]'::jsonb)))
    ...
    instruction_groups_json = CASE
      WHEN instruction_groups_json IS NULL OR instruction_groups_json = '[]'::jsonb THEN jsonb_build_array(jsonb_build_object('steps', (
        ...
        FROM jsonb_array_elements(COALESCE(instructions_json, '[]'::jsonb)) value
      )))
      ELSE instruction_groups_json
    END,
    tips_json = COALESCE(tips_json, '[]'::jsonb),
    substitutions_json = COALESCE(substitutions_json, '[]'::jsonb),
    tags_v2_json = CASE
      WHEN tags_v2_json IS NULL OR tags_v2_json = '{}'::jsonb THEN jsonb_build_object(
        'cuisine', COALESCE(cuisine, ''),
        'mealType', COALESCE(meal_type, ''),
        'dietary', COALESCE(dietary_flags_json, '[]'::jsonb),
        'season', NULL,
        'cookingMethod', '[]'::jsonb
      )
      ELSE tags_v2_json
