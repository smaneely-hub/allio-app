...
async function getRecipeFromCatalog(
  ...
  if (recipesWithData.length === 0) {
    console.log('[generate-plan] No recipes with complete data - using emergency fallback recipes')
    // Use hardcoded complete recipes as last resort
    ...
    console.log('[generate-plan] Using fallback recipe:', fallback.title)
    return {
      recipe: {
        name: fallback.title,
        description: fallback.description,
        prep_time_minutes: fallback.prep_time_minutes,
        cook_time_minutes: fallback.cook_time_minutes,
        servings: fallback.servings,
        ingredients: JSON.parse(fallback.ingredients_json),
        instructions: JSON.parse(fallback.instructions_json),
        cuisine: fallback.cuisine,
        recipe_id: fallback.id,
        from_recipe_library: false,
        weeknight_score: fallback.weeknight_score,
        kid_friendly_score: fallback.kid_friendly_score,
      },
      dietApplied: null
    }
  }
    ...
    if (HYBRID_MODE) {
      ...
      for (const slot of payload.slots) {
        ...
        if (recipe) {
          ...
          // Build meaningful why_this_meal from actual inputs
          const householdSize = payload.household?.total_people || 2
          const effort = slot.effort || 'medium'
          const hasKids = payload.members?.some((m: any) => m.age && m.age < 18) || false
          const dietFocus = payload.household?.diet_focus || dietApplied || ''
          
          let whyThis = `${recipe.cuisine} ${slot.meal || 'dinner'} with ${recipe.name}. `
          whyThis += `Matches your ${effort} effort level and ` + (hasKids ? 'family-friendly needs' : `${householdSize} servings`) + '.'
          if (dietFocus) whyThis += ` Aligned with your ${dietFocus} preference.`
          if (recipe.weeknight_score && recipe.weeknight_score > 7) whyThis += ' Quick enough for a busy weeknight.'
          
          // Contextual tips
          const tips = [
            `Prep: ${recipe.prep_time_minutes}min + Cook: ${recipe.cook_time_minutes}min`,
            recipe.kid_friendly_score > 5 && hasKids ? 'Good family option - kids typically enjoy this' : null,
            `Best with: ${recipe.cuisine} bread, salad, or roasted vegetables`
          ].filter(Boolean)
          
          // Contextual swaps
          const swaps = hasKids 
            ? ['Use milder spices if kids are sensitive', 'Cut vegetables smaller for easier eating', 'Serve with familiar sides']
            : ['Substitute with different protein', 'Add more aromatics for depth', 'Finish with fresh herbs']
