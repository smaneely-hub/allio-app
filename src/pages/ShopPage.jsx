      ...
      {Object.entries(groupedItems).map(([category, items]) => (
        <div key={category} className="card">
          <button type="button" onClick={() => setOpenCategories((current) => ({ ...current, [category]: !current[category] }))} className="flex w-full items-center justify-between text-left">
            <span className="font-display text-base md:text-lg text-warm-900 capitalize">{category}</span>
            <span className="text-xs md:text-sm text-warm-400">{openCategories[category] === false ? 'Show' : 'Hide'}</span>
          </button>
          {openCategories[category] !== false ? (
            <div className="mt-3 md:mt-4 space-y-2 md:space-y-3">
              {items
                ...
                .map((item) => (
                  <label key={item.id} className={`flex items-start gap-2 md:gap-3 rounded-xl md:rounded-2xl border p-2 md:p-4 text-xs md:text-sm ${item.checked ? 'border-warm-200 bg-warm-50 text-warm-400' : 'border-warm-200 bg-white text-warm-800'}`}>
                    <input type="checkbox" checked={Boolean(item.checked)} onChange={() => toggleItem(item.id)} className="mt-0.5 md:mt-1" />
                    <div className="min-w-0 flex-1">
                      <div className="font-medium truncate">{item.name}</div>
                      <div className="text-xs truncate">{item.quantity} {item.unit}</div>
                      <div className="mt-0.5 text-xs truncate hidden md:block">Used in: {(item.meals || []).join(', ')}</div>
                    </div>
                  </label>
                ))}
            </div>
