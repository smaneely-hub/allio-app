      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-text-secondary">
            {Number(item.protein_g || 0)}p / {Number(item.carbs_g || 0)}c / {Number(item.fat_g || 0)}f
            {showServings ? <span className="ml-2 text-text-muted">· {servings} servings</span> : null}
          </div>
