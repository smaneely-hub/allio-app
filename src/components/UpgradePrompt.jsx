...
/**
 * Paywall modal. Visibility is controlled by `feature` — pass null/undefined to hide.
 *
 * `onUpgrade` is called when the CTA is tapped. It should return { supported: boolean }.
 * When supported=false (no billing provider yet), falls back to navigating to /pricing.
 * When supported=true, the billing provider handled the purchase flow in-place.
 */
export function UpgradePrompt({ feature, onClose, onUpgrade }) {
  ...
  const handleUpgrade = async () => {
    if (onUpgrade) {
      ...
      if (result?.supported) return // billing provider handled the purchase flow
    }
    onClose?.()
    navigate('/pricing')
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-fadeIn overflow-hidden">
        ...
        <div className="text-center mb-4">
          <span className="font-display text-3xl text-text-primary">${PREMIUM_PRICE_MONTHLY}</span>
          <span className="text-text-muted">/month</span>
        </div>

        {/* CTA Button — routes through onUpgrade; falls back to /pricing when unsupported */}
        <button
